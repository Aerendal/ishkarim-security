"""
test_security.py — testy dla modułu ishkarim_security.
"""
import pytest
...
class TestImport:
    ...
    def test_import_main(self):
        ...
        assert ishkarim_security.__version__ == "0.1.0"

    def test_area(self):
        import ishkarim_security
        assert ishkarim_security.__area__ == "security"

    def test_modules_list_nonempty(self):
        import ishkarim_security
        assert len(ishkarim_security.MODULES) > 0

    def test_submodule_imports(self):
        from ishkarim_security import utils
...
class TestKnowledgeIndex:
    ...
    def test_returns_list(self, tmp_path):
        import ishkarim_security
        result = ishkarim_security.load_knowledge_index(root=str(tmp_path))
        assert isinstance(result, list)

    def test_record_keys(self, tmp_path):
        import os, ishkarim_security
        # create a minimal fake source dir
        src_dir = tmp_path / ishkarim_security.MODULES[0]
...
class TestUtils:
    ...
    def test_extract_python_blocks(self):
        from ishkarim_security.utils import extract_python_blocks
        md = """```python
def greet():
    return "hello world"
